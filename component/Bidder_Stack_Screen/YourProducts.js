...
{
  getDeviceInfo();
  const unsub = firestore().collection("Hb").onSnapshot((doc)=>{
...
<TouchableOpacity style={{marginTop:pi!==false?20:0}}
 onPress={()=>{props.navigation.navigate("View_YP",{pid:Pid,vid:vid})}} >
...
<allOther.vectorIcon.AntDesign name="check" color="green" size={25} />  
 ...
 {pi==false &&(
   <Text style={{fontSize:12,marginLeft:30,marginTop:10}}>
     Congratulations! You are highest bider of this product , Please click continue to process payment  
   </Text>
 )}

{(pi!==false && status=="pending" )&&(
   <Text style={{fontSize:12,marginLeft:30,marginTop:10}}>
     Your payment is processing .. Please wait for vendor approval and get you product after this approval  
   </Text>
 )}
   ...
   {pi==false &&(
    ...
    <TouchableOpacity
      onPress={()=>{ 
        setpid(pid);settp(price);setpn(pn);setpc(category);sethbid(id)
        setdialogClick(true);setdialogVisible(true)
      }}
     >      
     <Text style={{alignSelf:"center",color:"black",fontSize:16}}>Continue</Text>
    </TouchableOpacity>
      
            </View>
  )} 

{ (status=="deliver" && done==false) &&(
    <View style={{flex:1,height:40,justifyContent:"flex-end",marginTop:10,borderBottomRightRadius:10,borderBottomLeftRadius:10,alignItems:"center",justifyContent:"center",backgroundColor:"silver"}}>
    ...
    <TouchableOpacity
      ...
      Alert.alert(
        "Confirmation",
        "Do you want to confim Deleiver this product on your hand ",
        [
          {
            text: "Cancel",
    ...
    let resp =  await  firestore().collection("Hb").doc(id).update({done:true})
     if(resp){
      allOther.ToastAndroid.ToastAndroid_SB("Done")
     }
     setloader(false)
     ...
     >      
     <Text style={{alignSelf:"center",color:"black",fontSize:16}}>Confirmation</Text>
    </TouchableOpacity>
