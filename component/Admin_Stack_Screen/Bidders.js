...
setTimeout(() => {
 ...
 useEffect(()=>{
   console.log("all b d : ",allBiddersData);
 })
...
const blockVendor=  (id)=>{
  ...
  Alert.alert(
    "",
    "Are you sure ?  you want to Block this Bidder ?",
    [
      {
        text: "No",
      ...
      { text: "Yes", onPress: async () => {
        ...
        let resp =  await allOther.firebase.__Update(id,"block","bidder")
         
         if(resp){
           allOther.ToastAndroid.ToastAndroid_SB("Bidder Block Successful")
          } 
           setloader(false)
     ...
     <allOther.vectorIcon.FontAwesome5   style={{opacity:.7}}   size={25} color="black" name="angle-double-down" />
      ...
      const    RenderBidders  = () => { 
        ...
        let c=false; //for check empty vndrs or not
         let bidders  =   allBiddersData.bidders.map((item,index)=>{
...
<TouchableOpacity style={{marginTop:10}}
 onPress={()=>{props.navigation.navigate("ViewVendors",{vid:id,typ:"bidder"})}}>
...
onScroll={Animated.event([
 ...
 >
              ...
              {allBiddersData.bidders.length<=0   && !loader   
              ?(
